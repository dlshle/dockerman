...
func TestNginx(t *testing.T) {
	...
	t.Run("test current config w/out cfg", func(t *testing.T) {
		_, err := g.CurrentConfig(ctx, d, "nginx-gateway-test")
		if err != ErrGatewayNotDeployed {
			t.Errorf("expected error %v, got %v", ErrGatewayNotDeployed, err)
			t.FailNow()
		}
	})
	...
	t.Run("test create gateway", func(t *testing.T) {
		...
		gtwyContainer, err := g.GatewayContainerByNetwork(ctx, d, network)
		if err != nil {
			t.Fatal(err)
		}
		...
		if len(networks) == 0 {
			...
			t.Fatal("network already exists")
		}
		...
		currCfg, err := g.CurrentConfig(ctx, d, network)
		if err != nil {
